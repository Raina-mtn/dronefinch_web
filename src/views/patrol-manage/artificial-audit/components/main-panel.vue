<template>
  <div class="w-full h-full relative flex justify-center">
    ...
    <div v-if="isAudit" class="absolute bottom-16px z-100 w-[80%] flex-between">
      <el-button :disabled="currentIsFirst" color="rgba(0, 0, 0, 0.60)" @click="btnPre">
        <i class="iconfont icon-shangyizhang mr-2.5" />
        上一张
      </el-button>
      <el-button :disabled="currentIsLast" color="rgba(0, 0, 0, 0.60)" @click="btnNext">
        <i class="iconfont icon-xiayizhang mr-2.5" />
        下一张
      </el-button>
      <el-button color="rgba(0, 0, 0, 0.60)" @click="setMode('POLYGON')">
        <i class="iconfont icon-duobianxingxuanze mr-2.5" />
        多边形缺陷
      </el-button>
      <el-button color="rgba(0, 0, 0, 0.60)" @click="setMode('RECT')">
        <i class="iconfont icon-juxingxuanze mr-2.5" />
        矩形缺陷
      </el-button>
      <el-button color="rgba(0, 0, 0, 0.60)" @click="setMode('MARKER')">
        <i class="iconfont icon-a-tianjiaxiawu32808 mr-2.5" />
        添加备注
      </el-button>
      <el-button color="rgba(0, 0, 0, 0.60)" @click="recheck">
        <i class="iconfont icon-zhongxinjiance mr-2.5" />
        重新检测
      </el-button>
      <el-button color="rgba(0, 0, 0, 0.60)" @click="submit">
        <i class="iconfont icon-tuichu mr-2.5" />
        退出复核
      </el-button>
    </div>
  </div>
...
import {getImageRecheck} from "@/api/interface/patrol/audit";
...
const emit = defineEmits(["btnPre", "btnNext", "createInfo", "changePosition", "submit"]);
...
const setMode = (mode) => {
  ...
  switch (mode) {
  case "POLYGON": {
    drawingStyle.value = {strokeStyle: "#d70007", lineJoin: "round", lineCap: "round", lineWidth: 5};
    gMap.value.setDrawingStyle(drawingStyle.value);
    break;
  }
  case "RECT": {
    drawingStyle.value = {strokeStyle: "#d70007", lineWidth: 5};
    gMap.value.setDrawingStyle(drawingStyle.value);
    break;
  }
  case "MARKER": {
...
// 删除
const deleteGraphics = (type, id) => {
  if (type === "defete"){
    gFeatureLayer.value.removeFeatureById(`feature${id}`);//删除图形
    gTextLayer.value.removeTextById(`feature${id}`);//删除文字
  } else {
    gMap.value.markerLayer.removeMarkerById(`marker${id}`);//删除图形
    gTextLayer.value.removeTextById(`marker${id}`);//删除文字
  }
};
// 新增
const createInfo = (type, data, index) => {
  emit("createInfo", {type, data, index});
};
// 重新检测
const recheck = async () => {
  try {
    await getImageRecheck({id:props.currenCheck.id});
    bus.emit("getList");
  } catch (error) {
