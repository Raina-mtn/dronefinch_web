<template>
  <!-- 查询表单 card -->
  <SearchForm v-show="isShowSearch" :search="search" :reset="reset" :columns="searchColumns" :search-param="searchParam"
    :search-col="searchCol" :search-tool-button="searchToolButton">
    <template #searchHeader>
      <slot name="searchHeader">
        <el-button :icon="CirclePlus" @click="handleCreate" style="float: right; margin-bottom: 16px;"
  ...
  <div class="table-main">
    ...
    <div class="table-header">
      <div class="header-button-lf">
        <slot name="tableHeader" :selected-list-ids="selectedListIds" :selected-list="selectedList"
    ...
    <el-table ref="tableRef" v-bind="$attrs" :data="data ?? tableData" :border="border" :row-key="rowKey"
      ...
      <template v-for="item in tableColumns" :key="item">
        ...
        <el-table-column v-if="item.type && ['selection', 'index', 'expand','radio'].includes(item.type)" v-bind="item"
          :align="item.align ?? 'center'" :reserve-selection="item.type == 'selection'" :index="indexMethod">
      ...
      <template #empty>
        <div class="table-empty">
          <slot name="empty">
            <img src="@/assets/images/noData.png" alt="notData" class="mx-auto" />
            <div>暂无数据</div>
          </slot>
        </div>
      </template>
    ...
    <slot name="pagination">
      <div :class="[pagePosition === 'right' ? 'flex-row-reverse' : 'flex-row', 'mt-2', 'flex']">
        <Pagination v-if="pagination" :pageable="pageable" :handle-size-change="handleSizeChange"
          :handle-current-change="handleCurrentChange" small />
      </div>
    </slot>
  </div>
  <!-- 列设置 -->
...
import { Refresh, Operation, Search, CirclePlus } from "@element-plus/icons-vue";
...
const setEnumMap = async (col: ColumnProps) => {
  ...
  const { data:{list} } = await col.enum();
  enumMap.value.set(col.prop!, list);
};

const indexMethod = (index: number) => {
  const { pageIndex, pageSize } = unref(pageable);
